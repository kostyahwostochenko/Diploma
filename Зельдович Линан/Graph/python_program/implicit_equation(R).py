...
X_0_1 = np.linspace(0.005, 0.5, dot_cnt)
X_0_2 = np.linspace(0.005, 0.5, dot_cnt)
X_0_3 = np.linspace(0.005, 0.5, dot_cnt)
...
for i in range(0, len(R), 1):
    ...
    Le_F = 0.5
    X_0_1[i] = fsolve(func, 0.5, R[i])
    
    Le_F = 1
    X_0_2[i] = fsolve(func, 0.5, R[i])
    
    Le_F = 1.5
    X_0_3[i] = fsolve(func, 0.5, R[i])
...
#X_0_1 = 0.5*Le_X/((1-sigma)*(z*q*Le_X+2*R*(1-sigma)))
#X_0_2 = 1*Le_X/((1-sigma)*(z*q*Le_X+2*R*(1-sigma)))
#X_0_3 = 1.5*Le_X/((1-sigma)*(z*q*Le_X+2*R*(1-sigma)))

plt.plot(R, X_0_1, '--', color = 'r')
plt.plot(R, X_0_2, '--', color = 'b')
plt.plot(R, X_0_3, '--', color = 'g')
